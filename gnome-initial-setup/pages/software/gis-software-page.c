...
gis_software_page_locale_changed (GisPage *gis_page)
{
  ...
  g_object_set (priv->header, "subtitle", _("Third-party repositories provide access to additional software from selected external sources. They include popular apps, as well as firmware that is important for some devices. Some proprietary software is included, but this is recommended by default, but this is recommended by default."), NULL);
}
